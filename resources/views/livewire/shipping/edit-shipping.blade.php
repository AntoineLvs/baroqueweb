    ...
    <form wire:submit="submit">
        ...
        <div class="mt-6 sm:mt-5 space-y-6 sm:space-y-5">
            ...
            <div class="sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start sm:border-t sm:border-gray-200 sm:pt-5">
                ...
                <div class="mt-1 sm:mt-0 sm:col-span-2">
                    <select wire:model.defer="to_tenant_id" id="to_tenant_id"
                            class="max-w-lg block focus:ring-indigo-500 focus:border-indigo-500 w-full shadow-sm sm:max-w-xs sm:text-sm border-gray-300 rounded-md">
                        <option value="">-- Please choose --</option>
                        @foreach ($tenants as $to_tenant)
                            <option wire:key="{{ $to_tenant->id }}" value="{{ $to_tenant->id }}"
                                {{ $to_tenant->id == $to_tenant_id ? 'selected' : '' }}>
                                {{ $to_tenant->name }}
                            </option>
                        @endforeach
                    </select>
                    @error('to_tenant_id') <span class="text-sm text-red-500 mt-2">{{ $message }}</span> @enderror
                </div>
            </div>
