                      ...
                      <select wire:model="engine_id" name="engine_id" id="engine_id" class="max-w-lg block focus:ring-indigo-500 focus:border-indigo-500 w-full shadow-sm sm:max-w-xs sm:text-sm border-gray-300 rounded-md" required>
                          @foreach ($engines as $engine)
                              @if($engine['id'] == old('document'))
                                  <option wire:key="{{ $engine['id']}}" value="{{$engine->id }}" selected>{{$engine['name']}} ({{$engine['power_kw']}} kw / {{$engine['power_ps']}} ps) </option>
                              @else
                                  <option wire:key="{{ $engine['id']}}" value="{{$engine->id }}">{{$engine['name']}} ({{$engine['power_kw']}} kw / {{$engine['power_ps']}} ps)</option>
                              @endif
                          @endforeach
                      </select>
                  </div>
