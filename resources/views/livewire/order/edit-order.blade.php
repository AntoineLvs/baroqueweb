<div>
    ...
    <form wire:submit="submit">
        ...
        <div class="mt-8 flow-root">
            <!-- Product Selection -->
            <!-- New Version where we can select a product which is in the ordered Product, so the tenant can add a price per quantity to it. -->
            @if($loggedTenant->id == $toTenant->id)
            <div class="grid grid-cols-5 gap-4 mb-8">
                <div>
                    <label for="product" class="block text-sm font-medium text-gray-700">Product</label>
                    <select wire:model.live="selected_ordered_product_id" id="product"
                        ...
                        <option value="{{ $ordered_product->id }}">{{ $ordered_product->product->name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="number" wire:model="selected_product_quantity" id="quantity" min="1"
                        class="block w-full mt-1 rounded-md border-gray-300 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" disabled>
                </div>

                <div>
                    <label for="price" class="block text-sm font-medium text-gray-700">Price / Unit</label>
                    <input type="number" wire:model="selected_product_price" id="product_price" step="0.01"
                        class="block w-full mt-1 rounded-md border-gray-300 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                </div>

                <div>
                    <label for="tax" class="block text-sm font-medium text-gray-700">Tax</label>
                    <input type="number" wire:model="selected_product_tax" id="product_tax" step="0.01"
                        class="block w-full mt-1 rounded-md border-gray-300 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                </div>
